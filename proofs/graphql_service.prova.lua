--- Render-verification suite for the TypeScript GraphQL service archetype (GraphQL Yoga + pnpm +
...
for _, persistence in ipairs({ "PostgreSQL", "MySQL" }) do
  local label = "typescript-graphql[" .. persistence .. "]"
...
local none_project = prova.fixture("typescript-graphql[None]:project", Scope.File, function(ctx)
  return archetect.render{
    source = SRC,
...
prova.group("typescript-graphql[None]:image", { requires = { "docker" } }, function(g)
...
prova.group("typescript-graphql[None]:ci", { requires = { "docker" }, tags = { "standards" } }, function(g)
